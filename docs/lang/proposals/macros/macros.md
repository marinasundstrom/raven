...
* Split macro packages keep `ref/<tfm>` assemblies in the consumer metadata
  ...
  dependency probes.
* Freestanding macros now use `name!(...)` syntax, resolve through the same plugin registry, and support the same typed parameter-object binding direction as attached macros.
...
* Generated members, replacement declarations, and freestanding
  results participate in normal binding and code generation.
* Raw-body freestanding macros use `name!{ ... }`, preserve DSL
...
The current implementation composes attached declaration macros by source order and exposes both the original authored declaration and the in-flight declaration state to each macro.

For one annotated declaration:

1. each attached macro sees the original declaration through `TargetDeclaration`
2. each attached macro sees the declaration state immediately before it runs through `CurrentDeclaration`
3. if a macro returns `ReplacementDeclaration`, that replacement becomes the `CurrentDeclaration` for later macros on the same declaration
4. introduced members from all macros are integrated first, in macro source order
5. the declaration itself is kept or replaced, with the last `ReplacementDeclaration` taking effect
6. peer declarations from all macros are integrated last, in macro source order
...
This means the current model is intentionally a deterministic same-target pipeline with an explicit authored-syntax anchor, rather than an implicit whole-tree rewrite cascade.
