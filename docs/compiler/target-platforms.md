...
| Browser WebAssembly | Experimental | A framework-free Raven application composes `Raven.Sdk` with `Microsoft.NET.Sdk.WebAssembly`, starts the .NET runtime in a browser, and uses generated `[JSImport]` interop without Blazor. | Requires the .NET WebAssembly build tools. The first generator slice supports strings and string callbacks; interop macros and broader browser API projections remain future work. |
