...
| Unified nullability and pattern-first absence | `T?` is one declared nullable shape for reference and value types. Flow state is separate from that declared symbol. Raven recommends explicit patterns for handling nullable states and `Option<T>` when absence belongs to the domain; direct `is null` and `is not null` checks remain valid compatibility forms and currently narrow within their proven branch. | Gives nullability one source and semantic meaning without ignoring the CLR's different runtime encodings. It keeps domain absence explicit and exhaustive while allowing ordinary .NET interop and gradual migration. | Emit and consume standard .NET nullable annotations and flow attributes. Reference `T?` and value `Nullable<T>` remain conventional at the ABI even though Raven projects one model. **Keep the unified model and pattern-first guidance; treat diagnostic profiles as policy, not alternate semantics**. |
