...
| `RAV2704` | Error | Async return type must be Task-like | Async return type '{returnType}' is not supported; async functions must return System.Threading.Tasks.Task or System.Threading.Tasks.Task<T> | `samples/async/async-await.rav`, `samples/async/async-task-return.rav`, `samples/hello-world-func-async.rav` |
