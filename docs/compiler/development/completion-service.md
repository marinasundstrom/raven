...
- Offer pattern-oriented suggestions (e.g., `when` guards, `is`/`as` operators, `default`, `typeof`) when the caret is inside match arms or comparison patterns, leveraging the keyword set already tokenized by the lexer.【F:src/Raven.CodeAnalysis/Syntax/Tokens.xml†L45-L52】
