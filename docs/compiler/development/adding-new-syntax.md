...
Raven also supports Roslyn-style targeted formatting through `Formatter.Annotation`
and elastic trivia (`SyntaxFactory.ElasticSpace`, `SyntaxFactory.ElasticLineFeed`,
and friends). Keep `SyntaxNormalizer` and `Formatter` aligned: full normalization
should still work on detached raw nodes, while annotation-aware formatting should
only rewrite annotated regions and elastic trivia.
