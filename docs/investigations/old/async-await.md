...
* `UsingTryAwaitExpressionAsyncAssembly_PassesIlVerifyWhenToolAvailable` adds another IL-level regression that mimics the `samples/async/http-client.rav` disposal pattern, keeping awaits that resume inside stacked `use let` statements under verifier coverage.【F:test/Raven.CodeAnalysis.Tests/CodeGen/AsyncILGenerationTests.cs†L309-L346】
