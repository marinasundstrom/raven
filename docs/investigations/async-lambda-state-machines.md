...
- [x] **Fix async block-lambda inference**: Ensure block-bodied async lambdas flow their collected `return` types (e.g., `int` from `return 42`) into async return shaping so the inferred delegate becomes `Func<Task<int>?>` instead of defaulting to `Func<Task?>`/`Func<Task<T>>` with an unresolved `T`.
- [x] **Stabilize generic substitution in async lambdas**: Track where `TResult` remains unresolved during bound-tree/codegen for `Task.Run(async () => { ... })` and ensure the async rewriter and state-machine synthesis operate on constructed delegate types rather than open generic placeholders.
- [x] **Trace async lambda compatibility in `TryMatch`**: Use overload logs and replay instrumentation to see why `Task.Run(Func<Task<TResult>?>)` reports `ArgumentMismatch` for the block-bodied async lambda even when the argument is typed as `Func<Task<int>>`, and adjust the replay/compatibility checks accordingly.
- [x] **Re-check overload resolution after inference fix**: After inference/substitution changes, rerun `samples/async/async-inference.rav -bt` to confirm `Task.Run` binds to `Task<TResult> Run<TResult>(Func<Task<TResult>?>)` and that `WriteLine` resolves without ambiguity; add regression coverage for the block-bodied form.
...
### Findings after step 34
- Added collection of async return inference during lambda type-argument inference so block-bodied async lambdas feed their collected async returns (e.g., `Task<int>` from `return 42`) into generic substitutions before falling back to raw body types.
- Rerunning `dotnet run --project src/Raven.Compiler -- samples/async/async-inference.rav -bt` still crashes in codegen when projecting the block async lambda’s delegate type (`Unable to resolve runtime type for type parameter: TResult`), so the lambda continues to reach emission with an open `TResult` despite the collected async return being available.【b56c8c†L1-L62】
...
### Findings after step 40
- Added per-parameter overload comparison logging so `--overload-log` now records how each argument matches candidate parameters (including conversion kinds and lambda compatibility). For the block-bodied `Task.Run` call the async lambda argument is still typed as `Func<Task<int>>`, but both task-returning overloads are rejected as `LambdaIncompatible` by the binder callback, exposing the parameter/argument comparison that prevents the async delegate overload from binding and keeping the call in `ArgumentMismatch`.【716eae†L33-L60】
- The new comparison logs also spell out why the subsequent `WriteLine` call is ambiguous by listing each numeric overload with `identity` matches against the `Error` expression result, clarifying which overload comparisons drive the ambiguity.【716eae†L60-L110】
