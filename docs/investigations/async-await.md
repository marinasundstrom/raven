...
- Return statements now detect when the compiler-synthesized `Task.CompletedTask` placeholder flows through a parameterless builder and treat it as an effectively-void return, ensuring the state machine still stamps `_state = -2` and calls `SetResult()` so runtimes observing the builder task see completion instead of hanging on the implicit fall-through.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L328-L354】
...
- Add regression coverage for explicit and implicit `Task.CompletedTask` paths so parameterless builders always observe a `SetResult()` even when the original Raven code fell through without a `return`, keeping future lowering tweaks from regressing the runtime hang fix.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L328-L354】【db673f†L1-L4】
