...
* Await lowering now remaps `GetAwaiter`, `get_IsCompleted`, and `GetResult` through the async-method substitution before emitting the calls, so the MethodSpecs reference the async method’s `!!0` rather than the state machine’s `!0`.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L1338-L1375】
