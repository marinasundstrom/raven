...
* Await lowering now runs each awaited expression through an async-method substituter before rewriting so the bound tree stops pointing at state-machine-only symbols when we build `GetAwaiter` and `GetResult` invocations.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L572-L642】 The generated IL for `Program/'<>c__AsyncStateMachine1'::MoveNext` still shows `TaskAwaiter<!0>` on the awaiter call, so the substitution is not yet flowing through to the MethodSpec metadata we emit.【4ea04d†L1-L47】
