...
- The emitted state-machine IL still mishandles the `_builder` field: lowering introduces `BoundMemberAccessExpression` nodes for `_builder.Start`, `_builder.SetResult`, `_builder.SetException`, and `_builder.SetStateMachine`, and the emitter translates those into `ldfld` + temporary locals for value-type receivers. As a result the runtime mutates a copy of `AsyncTaskMethodBuilder` instead of the field stored on the state machine, so the generated IL never updates the actual builder and surfaces as "bad IL" when the method executes.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L1550-L1615】【F:src/Raven.CodeAnalysis/CodeGen/Generators/ExpressionGenerator.cs†L1965-L2213】
