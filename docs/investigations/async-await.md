...
1. `AwaitLoweringRewriter` now records the enclosing `try` block for each `StateDispatch` entry (using `_tryBlocks`) so it knows when a resume label lives inside a guard. The dispatch metadata stores that `BoundBlockStatement` alongside the state and label.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L700-L760】【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L1447-L1503】【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L2031-L2045】
