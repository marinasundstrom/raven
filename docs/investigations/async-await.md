...
1. **Propagate the awaited result type into state-machine synthesis.** Extend `AsyncLowerer` and the synthesized state-machine symbol to flow the method’s `Task<T>` return type through to the builder field so value-returning async methods request `AsyncTaskMethodBuilder<T>` instead of the unbound `AsyncTaskMethodBuilder<!0>`. Confirm that local functions, property/indexer accessors, and top-level `MainAsync` bodies receive the same specialization.【F:docs/investigations/snippets/async-return-value-raven.il†L1-L32】
2. **(In progress)** Specialize builder member lookups at emission time. `ExpressionGenerator` now resolves async-builder invocations against the constructed builder symbol so the runtime sees the closed generic overloads for `Start`, `SetException`, and `SetResult`. The new IL regression test validates that the generated `MoveNext` body calls `AsyncTaskMethodBuilder<int>.SetResult(int)` rather than the open generic placeholder. Follow-up work: re-run the failing repro to confirm the runtime no longer throws and capture the before/after disassembly for the investigation.【F:src/Raven.CodeAnalysis/CodeGen/Generators/ExpressionGenerator.cs†L2507-L2607】【F:test/Raven.CodeAnalysis.Tests/CodeGen/AsyncILGenerationTests.cs†L239-L274】
3. **Validate end-to-end runtime behaviour.** Re-run the failing sample via `dotnet run -- samples/test7.rav -o test.dll -d pretty && dotnet test.dll` once the emitter changes land, add a regression harness that loads the produced assembly via reflection, and document the successful execution path to keep the investigation notes current.【c5d584†L1-L8】
