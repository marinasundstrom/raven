...
* Await lowering now runs each awaited expression through an async-method substituter before rewriting so the bound tree stops pointing at state-machine-only symbols when we build `GetAwaiter` and `GetResult` invocations.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L572-L642】 We now keep the awaited helper methods and properties wired to the async method’s original definitions while emitting the state-machine field stores so the MethodSpecs reference the method’s `!!0` generics instead of the struct placeholders.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L1380-L1404】【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L1422-L1435】 The generated IL for `Program/'<>c__AsyncStateMachine1'::MoveNext` still shows `TaskAwaiter<!0>` on the awaiter call, so the remaining mismatch is likely in how Reflection.Emit materializes the MethodSpec.【4ea04d†L1-L47】
...
* Despite the constructor-free initialization, loading the sample assembly still fails with `BadImageFormatException`, so the remaining mismatch likely sits in the MethodSpec metadata rather than the state machine layout.【c1f159†L1-L9】
...
* Inspect the emitted MethodSpecs for `Program/'<>c__AsyncStateMachine1'::MoveNext` so we understand why the awaiter call still materializes `TaskAwaiter<!0>` even after running the async-method substituter over the bound expression.【F:src/Raven.CodeAnalysis/BoundTree/Lowering/AsyncLowerer.cs†L572-L642】【4ea04d†L1-L47】
