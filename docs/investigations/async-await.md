...
* Added regression coverage for `AsyncTaskMethodBuilder<T>.SetResult`, which now decodes to a MemberRef over the state-machine generic parameter (`!0`), matching Roslyn and ruling out the completion call as the `BadImageFormatException` trigger.【F:test/Raven.CodeAnalysis.Tests/Semantics/AsyncLowererTests.cs†L1195-L1252】【e3940d†L1-L30】
...
* Trace any remaining MethodSpec mismatches—`Start`, `AwaitUnsafeOnCompleted`, and `SetResult` now line up, but we still need to audit `SetException` and the awaiter helpers—to ensure Reflection.Emit never falls back to the state-machine generic parameter builders.
...
* Inspect the custom attributes (`AsyncStateMachineAttribute`, `AsyncMethodBuilderAttribute`) and their MemberRefs to ensure their type arguments mirror Roslyn’s output and aren’t tripping the verifier.
