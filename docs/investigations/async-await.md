...
- The IL generator now emits `ldarga.s 0` when passing the synthesized state machine to the async builder helpers, ensuring `AwaitUnsafeOnCompleted` receives a concrete receiver address instead of an unspecified operand. The call site still merges the awaiter and state-machine type arguments into a malformed method spec, so the CLR continues to reject the method until we correct the metadata emission.【F:src/Raven.CodeAnalysis/CodeGen/Generators/ExpressionGenerator.cs†L905-L926】【F:src/Raven.CodeAnalysis/CodeGen/Generators/ExpressionGenerator.cs†L2210-L2276】
