...
* Fix the `Task.FromResult` inference/regression so `async-await.rav` compiles again, then re-enable it in the sample regression matrix.【55c933†L1-L11】
* Allow `catch (Exception)` to bind against `System.Exception` itself so the `async-try-catch.rav` sample no longer throws `RAV1016`.【19464a†L1-L4】
