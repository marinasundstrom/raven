...
3. ☐ **Binding to symbols**
   ...
   - Progress: Leading documentation trivia is captured and merged into a single documentation payload per source symbol using the syntax tree’s documentation format (Markdown by default), preserving per-declaration content for future diagnostics and inheritance work across partial declarations.
4. ☐ **Diagnostics**
   ...
   - Progress: Documentation comments are validated per syntax tree using the selected format, reporting unattached comments and malformed XML or Markdown content while `DocumentationMode` is enabled.
...
- **Markdown output**: emit one Markdown file per named type so large surfaces stay navigable and incremental outputs remain stable. Each file mirrors the type hierarchy using headings for namespaces, the containing type, and members (e.g., `# Namespace`, `## Type`, `### Member`) and contains the Markdown payload from `GetDocumentationComment` beneath each heading. Generate an ad hoc index Markdown file alongside the outputs that lists every emitted type file (sorted deterministically) with links, and preserve declaration order within a type to keep related overloads grouped while still allowing alphabetical or ID-based sorting when needed for stability. Allow the output path to be specified so tooling can write into a docs directory.
