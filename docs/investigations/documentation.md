The `BaseParseContext` doesn't handle document comments (///) correctly. 

The goal is to produce a valid trivia.
...
See if you can use some of the knowledge below.

This block parsing the documentation comment:

```csharp
// ✅ NEW: read contiguous /// lines as one trivia (and consume newline tokens)
...
internal class BaseParseContext : ParseContext
{
    ...
    private SyntaxToken ReadTokenCore()
    {
        ...
        if (TreatNewlinesAsTokens && ShouldPromoteToNewlineToken() && IsRawNewlineTokenKind(token.Kind))
        {
            _lexer.ReadToken();
            return MakeNewlineToken(token);
        }
    ...
    private SyntaxToken MakeNewlineToken(Token t)
    {
        return new SyntaxToken(
            ...
            diagnostics: t.GetDiagnostics());
    }

    private static bool IsRawNewlineTokenKind(SyntaxKind k)
    {
        return k is SyntaxKind.NewLineToken
        ...
        while (true)
        {
            ...
            if (token.Kind == SyntaxKind.SlashToken)
            {
                ...
                if (token2.Kind == SyntaxKind.SlashToken)
                {
                    ...
                    // existing single line comment logic (//, or /// in trailing trivia)
                ...
                // /* ... */ and /** ... */ unchanged...
                if (token2.Kind == SyntaxKind.StarToken)
                {
    ...
    private void ReadSingleLineDocCommentBlockInto(StringBuilder sb)
    {
        while (true)
        {
            // allow indentation before /// on each line
            ...
            // ✅ IMPORTANT: consume newline tokens even when newlines are "significant"
            ConsumeOneNewlineInto(sb);

            // continue only if next line (after indentation) starts with ///
...
Test program in project TestApp. Method: Docs