...
sealed class SolutionState
{
    ...
    private ProjectState CreateProjectState(ProjectInfo pi)
    {
        return new ProjectState(pi,
            new TextDocumentStates<DocumentState>(
                pi.Documents.Select(di => CreateDocumentState(di)));
    }
    ...
    {
        var newAnalyserReference = AnalyzerReferences.Concat(analyzerReferences);
        return new SolutionState(WorkspaceKind, WorkspaceVersion, SolutionAttributes, ProjectIds, ProjectStates, newAnalyserReference.ToList());
    }
}
