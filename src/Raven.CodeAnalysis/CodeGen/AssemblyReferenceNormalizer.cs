        ...
        if (method is not PEMethodSymbol && method.OriginalDefinition is PEMethodSymbol definition)
        {
            ...
            if (method.IsGenericMethod)
            {
                ...
                foreach (var argument in method.TypeArguments)
                    constructed.GenericArguments.Add(CreateTypeReference(module, argument, targetReferences, context, asGenericArgument: true));
                return constructed;
            }
            return imported;
        }
        ...
        bool asGenericArgument = false)
    {
            ...
            !SymbolEqualityComparer.Default.Equals(constructed, definition))
        {
            ...
            foreach (var argument in constructed.TypeArguments)
                generic.GenericArguments.Add(CreateTypeReference(module, argument, targetReferences, context, asGenericArgument: true));
            return generic;
        }
