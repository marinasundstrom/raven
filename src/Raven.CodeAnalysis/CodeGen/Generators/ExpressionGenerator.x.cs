...
internal partial class ExpressionGenerator
{
    ...
    Type resultClrType)
    {
        // We are in the error path (TryGetOk was false). Extract the error payload from the receiver
        // and wrap it into Result<U,E>.Error, then implicitly convert to Result<U,E>.

        // Find Error case type + ctor on the RESULT type we're producing.
        var errorCaseType = FindNestedCaseTypeSafe(resultClrType, "Error");

        // Prefer a single-arg ctor; fall back to parameterless.
        const BindingFlags ctorFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
        ConstructorInfo? errorCtor;
        try
        {
            errorCtor = errorCaseType.GetConstructors(ctorFlags).FirstOrDefault(c => c.GetParameters().Length == 1)
                ?? errorCaseType.GetConstructors(ctorFlags).FirstOrDefault(c => c.GetParameters().Length == 0);
        }
        catch (NotSupportedException)
        {
            // Constructed generic types from Reflection.Emit can throw; map from definition.
            var def = errorCaseType.IsGenericType ? errorCaseType.GetGenericTypeDefinition() : errorCaseType;
            var defCtor = GetConstructorsSafe(def, ctorFlags).FirstOrDefault(c => c.GetParameters().Length == 1)
                ?? GetConstructorsSafe(def, ctorFlags).FirstOrDefault(c => c.GetParameters().Length == 0)
                ?? throw new InvalidOperationException("Missing Error ctor");
            errorCtor = MapCtor(errorCaseType, defCtor);
        }

        if (errorCtor is null)
            throw new InvalidOperationException("Missing Error ctor");

        // Parameterless Error: just construct the case.
        if (errorCtor.GetParameters().Length == 0)
        {
            ILGenerator.Emit(OpCodes.Newobj, errorCtor);
            var opImplicit0 = FindImplicitConversionSafe(resultClrType, errorCaseType);
            ILGenerator.Emit(OpCodes.Call, opImplicit0);
            return;
        }

        // Single-arg Error: extract the error payload from the carrier (receiver).
        var paramType = Generator.InstantiateType(errorCtor.GetParameters()[0].ParameterType);

        // Find TryGetError(out ...)
        var tryGetError = FindSingleOutBoolMethod(receiverClrType, "TryGetError")
            ?? throw new InvalidOperationException("Missing TryGetError");

        var errorOutType = Generator.InstantiateType(tryGetError.GetParameters()[0].ParameterType.GetElementType()!);
        var errorOutLocal = ILGenerator.DeclareLocal(errorOutType);

        var gotErrorLabel = ILGenerator.DefineLabel();
        var doneLabel = ILGenerator.DefineLabel();

        // if (tmp.TryGetError(out errorOutLocal)) goto gotErrorLabel; else fall back to default error payload
        ILGenerator.Emit(OpCodes.Ldloca_S, receiverTmp);
        ILGenerator.Emit(OpCodes.Ldloca_S, errorOutLocal);
        ILGenerator.Emit(OpCodes.Call, tryGetError);
        ILGenerator.Emit(OpCodes.Brtrue, gotErrorLabel);

        // Fallback (should be rare): default(TParam)
        if (paramType.IsValueType)
        {
            var tmpDefault = ILGenerator.DeclareLocal(paramType);
            ILGenerator.Emit(OpCodes.Ldloca_S, tmpDefault);
        ...
        // Normal: load the extracted error value
        ILGenerator.MarkLabel(gotErrorLabel);

        if (errorOutType == paramType)
        {
            ILGenerator.Emit(OpCodes.Ldloc, errorOutLocal);
        }
        else
        {
            // When reading payload from errorOutLocal:
            var valueGetter =
                errorOutType.GetProperty("Data", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod ??
                errorOutType.GetProperty("Value", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod ??
                errorOutType.GetProperty("Error", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod ??
                errorOutType.GetProperty("Message", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod ??
                errorOutType.GetProperty("Payload", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod ??
                errorOutType.GetProperty("Item", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)?.GetMethod;

            if (valueGetter is null)
                throw new InvalidOperationException("Missing Error payload property (Data/Value/Error/Message/Payload/Item)");

            ILGenerator.Emit(OpCodes.Ldloca_S, errorOutLocal);
            ILGenerator.Emit(OpCodes.Call, valueGetter);

            // Convert to ctor param type if needed
            var getterReturnType = valueGetter.ReturnType;
            if (paramType != getterReturnType)
            {
                if (paramType.IsValueType)
                {
                    // Only legal automatic path here is: object/ref -> valuetype via unbox.any
                    ...
                    {
                        // Don’t guess value-type conversions here.
                        throw new InvalidOperationException(
                            $"Cannot convert Error payload from '{getterReturnType}' to '{paramType}'.");
                    }
                }
                else
                {
                    // Reference type: normal cast
        ...
        // implicit conversion Error -> Result
        var opImplicit = FindImplicitConversionSafe(resultClrType, errorCaseType);
        ILGenerator.Emit(OpCodes.Call, opImplicit);
    }
