...
internal class ExpressionGenerator : Generator
{
    ...
    public MethodInfo GetMethodInfo(IMethodSymbol methodSymbol)
    {
        ...
        if (methodSymbol is ConstructedMethodSymbol constructedMethod)
        {
            ...
            var resolvedArguments = constructedMethod.TypeArguments
                .Select(arg => ResolveClrType(arg!))
                .ToArray();

            if (resolvedArguments.Any(RequiresRuntimeType))
            {
                var runtimeDefinition = EnsureRuntimeMethod(genericDefinition);
                var runtimeGenericDefinition = runtimeDefinition.IsGenericMethodDefinition
                    ? runtimeDefinition
                    : runtimeDefinition.GetGenericMethodDefinition();

                var runtimeArguments = constructedMethod.TypeArguments
                    .Select(arg => EnsureRuntimeType(ResolveClrType(arg!)))
                    .ToArray();

                return runtimeGenericDefinition.MakeGenericMethod(runtimeArguments);
            }

            return genericDefinition.MakeGenericMethod(resolvedArguments);
        }
    ...
    private static MethodInfo EnsureRuntimeMethod(MethodInfo methodInfo)
    {
        if (!IsMetadataLoadContextMember(methodInfo))
            return methodInfo;

        var declaringType = EnsureRuntimeType(methodInfo.DeclaringType!);
        var metadataParameters = methodInfo.GetParameters();
        var parameterTypes = methodInfo.IsGenericMethodDefinition
            ? null
            : metadataParameters
                .Select(p => EnsureRuntimeType(p.ParameterType))
                .ToArray();

        var candidateMethods = declaringType
            .GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
            .Where(m => m.Name == methodInfo.Name)
            .Where(m => m.IsGenericMethodDefinition == methodInfo.IsGenericMethodDefinition)
            .Where(m => (!m.IsGenericMethodDefinition && !methodInfo.IsGenericMethodDefinition) || m.GetGenericArguments().Length == methodInfo.GetGenericArguments().Length)
            .Where(m => methodInfo.IsGenericMethodDefinition
                ? m.GetParameters().Length == metadataParameters.Length
        ...
        if (candidateMethods.Count > 1)
        {
            // Prefer the overload whose return type matches the metadata return type.
            var metadataReturnType = EnsureRuntimeType(((MethodInfo)methodInfo).ReturnType);
            var matchingReturn = candidateMethods
                .Where(m => TypesEquivalent(EnsureRuntimeType(m.ReturnType), metadataReturnType))
                .ToList();

            if (matchingReturn.Count == 1)
                return matchingReturn[0];

            candidateMethods = matchingReturn;
        }

        if (candidateMethods.Count == 0)
            throw new InvalidOperationException($"Unable to resolve runtime method for '{methodInfo}'.");

        throw new InvalidOperationException($"Multiple runtime method matches found for '{methodInfo}'.");
    }

    private static bool ParametersMatch(ParameterInfo[] candidateParameters, Type[] expectedTypes)
    {
        if (candidateParameters.Length != expectedTypes.Length)
            return false;
        ...
        for (var i = 0; i < candidateParameters.Length; i++)
        {
            ...
            if (!TypesEquivalent(candidateType, expectedType))
                return false;
        }
    ...
    private static bool TypesEquivalent(Type left, Type right)
    {
        ...
        if (left.IsGenericType || right.IsGenericType)
        {
            ...
            if (!TypesEquivalent(leftDef, rightDef))
                return false;
            ...
            if (!left.IsGenericTypeDefinition)
            {
                ...
                {
                    if (!TypesEquivalent(leftArgs[i], rightArgs[i]))
                        return false;
                }
            }
    ...
    private static bool IsMetadataLoadContextMember(MemberInfo memberInfo)
    {
        var memberType = memberInfo.GetType();
        return memberType.Namespace == "System.Reflection.TypeLoading" ||
               memberType.FullName?.StartsWith("System.Reflection.TypeLoading.") == true;
    }

    private static bool RequiresRuntimeType(Type type)
    {
        if (type is null)
            return false;

        if (type is TypeBuilder or System.Reflection.Emit.EnumBuilder)
            return true;

        if (type.Assembly is AssemblyBuilder)
            return true;

        var assemblyTypeName = type.Assembly.GetType().FullName;
        return assemblyTypeName == "System.Reflection.Emit.AssemblyBuilder";
    }
    ...
    private static Type EnsureRuntimeType(Type type)
    {
        ...
        {
            var def = EnsureRuntimeType(type.GetGenericTypeDefinition());
            var args = type.GetGenericArguments().Select(EnsureRuntimeType).ToArray();
        ...
        var assemblyTypeName = type.Assembly.GetType().FullName;
        if (assemblyTypeName == "System.Reflection.RuntimeAssembly")
            return type;

        if (type.AssemblyQualifiedName is string qualifiedName)
        {
            var runtimeType = Type.GetType(qualifiedName, throwOnError: false);
