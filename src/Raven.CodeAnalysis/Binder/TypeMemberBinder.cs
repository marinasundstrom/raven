...
internal partial class TypeMemberBinder : Binder
{
        ...
        SyntaxReference declarationReference)
    {
        ...
        foreach (var decl in declaration.Declarators)
        {
            ...
            if (decl.Initializer is not null)
            {
                ...
                initializer = fieldType is null
                    ? exprBinder.BindExpression(decl.Initializer.Value)
                    : exprBinder.BindExpressionWithTargetTypeForSemanticQuery(decl.Initializer.Value, fieldType);
    ...
    public Dictionary<AccessorDeclarationSyntax, Binder> BindEventDeclaration(EventDeclarationSyntax eventDecl)
    {
        ...
        if (isVirtual && !isOverride && _containingType.IsClosed)
        {
            _diagnostics.ReportVirtualMemberInClosedType(eventName, _containingType.Name, identifierToken.GetLocation());
            isVirtual = false;
        }
    ...
    public Dictionary<AccessorDeclarationSyntax, Binder> BindIndexerDeclaration(IndexerDeclarationSyntax indexerDecl)
    {
        ...
        if (isVirtual && !isOverride && _containingType.IsClosed)
        {
            _diagnostics.ReportVirtualMemberInClosedType("Item", _containingType.Name, identifierToken.GetLocation());
            isVirtual = false;
        }
