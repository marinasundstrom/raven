...
/// <summary>
/// Represents an array type within the metadata-only reflection universe.
/// </summary>
internal sealed class MetadataArrayType : TypeInfo
{
    private readonly Type _elementType;
    private readonly int _rank;

    public MetadataArrayType(Type elementType, int rank)
    {
    ...
    public override string Name => _elementType.Name + GetSuffix();

    public override string? FullName => _elementType.FullName is null ? null : _elementType.FullName + GetSuffix();

    public override string? AssemblyQualifiedName
        => FullName is null ? null : $"{FullName}, {Assembly.FullName}";

    public override Module Module => _elementType.Module;

    public override Type? BaseType => _elementType.Module.Assembly.GetType("System.Array", throwOnError: false, ignoreCase: false) ?? typeof(Array);
    ...
    public override Type GetGenericTypeDefinition() => throw new InvalidOperationException("Array types are not generic.");

    public override Type MakeGenericType(params Type[] typeArguments) => throw new InvalidOperationException("Array types are not generic.");
    ...
    public override bool IsAssignableFrom(Type? c) => c is MetadataArrayType other && other._elementType == _elementType && other._rank == _rank;
