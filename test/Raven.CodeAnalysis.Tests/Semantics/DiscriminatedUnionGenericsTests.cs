...
""";
    ...
    public void GenericUnionPattern_EmitsClosedCaseTypes()
    {
        ...
        var typeNames = DecodeTypeSpecificationNames(metadata);

        var resultSpecs = typeNames
            .Where(name => name.Contains("Result`1", StringComparison.Ordinal))
            .ToList();

        Assert.NotEmpty(resultSpecs);
        Assert.Contains(resultSpecs, name => name.Contains("Int32", StringComparison.Ordinal));
        Assert.Contains(resultSpecs, name => name.Contains("Ok", StringComparison.Ordinal) && name.Contains("Int32", StringComparison.Ordinal));
        Assert.Contains(resultSpecs, name => name.Contains("Error", StringComparison.Ordinal) && name.Contains("Int32", StringComparison.Ordinal));
    }
