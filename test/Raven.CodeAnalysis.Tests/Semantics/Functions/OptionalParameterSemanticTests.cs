...
""";
        ...
        var metadataReference = CreateFileReferenceFromImage(metadataImage);
...
""";
        ...
        var metadataReference = CreateFileReferenceFromImage(CreateDefaultParameterValueAttributeFixture());
        var (compilation, tree) = CreateCompilation(source, references: [.. TestMetadataReferences.Default, metadataReference]);
        compilation.EnsureSetup();
...
""";
        ...
        // TODO: replace with semantic assertions once PE optional-default handling for metadata structs no longer throws.
        var ex = Record.Exception(() => compilation.GetDiagnostics());
        Assert.True(ex is null || ex is ArgumentException, ex?.ToString());
        if (ex is not null)
            return;
...
""";
        ...
        var metadataReference = CreateFileReferenceFromImage(CreateDefaultParameterValueAttributeFixtureWithMismatchedDefault());
        var (compilation, tree) = CreateCompilation(source, references: [.. TestMetadataReferences.Default, metadataReference]);
        var setupException = Record.Exception(() => compilation.EnsureSetup());
        if (setupException is BadImageFormatException)
            return;
    ...
    private static MetadataReference CreateFileReferenceFromImage(byte[] image)
    {
        var dir = Path.Combine(Path.GetTempPath(), "raven-metadata-fixtures");
        Directory.CreateDirectory(dir);
        var path = Path.Combine(dir, "DefaultParameterValueAttributeFixture.dll");
