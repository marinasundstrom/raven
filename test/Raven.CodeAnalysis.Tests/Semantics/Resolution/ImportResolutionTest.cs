...
public class ImportResolutionTest : DiagnosticTestBase
{
    ...
    public void NamespaceSystemNotImportedStringNotAvailable_Should_ProduceDiagnostic()
    {
        ...
        var verifier = CreateVerifier(
                    testCode,
                    [
                         new DiagnosticResult(CompilerDiagnostics.TheNameDoesNotExistInTheCurrentContext.Id).WithSpan(1, 1, 1, 7).WithArguments("String")
                    ]);
    ...
    public void NamespaceSystemNotImportedStringInClassNotAvailable_Should_ProduceDiagnostic()
    {
        ...
        var verifier = CreateVerifier(
            testCode,
            [
                new DiagnosticResult(CompilerDiagnostics.TheNameDoesNotExistInTheCurrentContext.Id).WithSpan(2, 16, 2, 22).WithArguments("String")
            ]);
    ...
    public void NamespaceImportWithoutWildcard_Should_ProduceDiagnostic()
    {
        ...
        var verifier = CreateVerifier(
            testCode,
            [
                new DiagnosticResult(CompilerDiagnostics.TypeExpectedWithoutWildcard.Id).WithSpan(1, 8, 1, 14),
                new DiagnosticResult(CompilerDiagnostics.TheNameDoesNotExistInTheCurrentContext.Id).WithSpan(3, 1, 3, 7).WithArguments("String"),
            ]);
