...
public class NamespaceResolutionTest : DiagnosticTestBase
{
    ...
    public void ConsoleDoesNotContainWriteLine2_Should_ProduceDiagnostics()
    {
        ...
        var verifier = CreateVerifier(
                    testCode,
                    [
                         new DiagnosticResult(CompilerDiagnostics.MemberDoesNotContainDefinition.Id).WithSpan(1, 16, 1, 26).WithArguments("Console", "WriteLine2"),
                    ]);
    ...
    public void ConsoleDoesNotExistInCurrentContext_Should_ProduceDiagnostics()
    {
        ...
        var verifier = CreateVerifier(
                    testCode,
                    [
                         new DiagnosticResult(CompilerDiagnostics.TheNameDoesNotExistInTheCurrentContext.Id).WithSpan(1, 1, 1, 8).WithArguments("Console"),
                    ]);
