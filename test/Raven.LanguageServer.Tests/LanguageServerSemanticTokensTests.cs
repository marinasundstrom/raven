...
""");
    ...
    public async Task SemanticTokens_ClassifyBaseExpressionMemberAsync()
    {
        const string code = """
open class App {
    public virtual func Run() -> string {
        return "base"
    }
}
...
""";
        ...
        try
        {
            ...
            var existingBefore = await store.TryEnterExistingDocumentSemanticModelAccessAsync(
                uri,
                CancellationToken.None,
                "test");
            existingBefore.ShouldBeNull();
