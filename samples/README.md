...
| `discriminated-unions.rav` | ❌ Fails | Shorthand case construction (`.Ok(...)`, `.Error(...)`) still binds as a member-binding invocation, so `BindInvocationExpression` falls back to calling `Invoke` on the case type (`RAV0117`). The match arms use the same leading-dot syntax, but the pattern binder only recognizes declaration/tuple/unary/binary forms, so they trip `RAV1001`/`RAV0103` before exhaustiveness analysis. The file now keeps its union declarations after the global statements so it satisfies the existing ordering rule while we focus on binding, lowering, and code generation. |
