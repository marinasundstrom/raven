...
| `discriminated-unions.rav` | ❌ Fails | Shorthand case construction (`.Ok(...)`, `.Error(...)`) still binds as a member-binding invocation, so `BindInvocationExpression` falls back to calling `Invoke` on the case type (`RAV0117`). The match arms use the same leading-dot syntax, but the pattern binder only recognizes declaration/tuple/unary/binary forms, so they trip `RAV1001`/`RAV0103` before exhaustiveness analysis. Top-level statements can now interleave with the union declarations, so the remaining work is focused on binding, lowering, and code generation. |
...
3. ✅ Allow top-level `let` statements and `Console.WriteLine` calls to coexist with the union declarations without triggering `RAV1011` by letting `CreateTopLevelBinder` skip the `FileScopedCodeOutOfOrder` diagnostic. 【F:src/Raven.CodeAnalysis/SemanticModel.cs†L579-L610】
