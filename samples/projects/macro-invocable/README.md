...
- The query implements `IMacroFragmentProvider` and reports its source,
  ...
  editor provider. It also creates a sequence-element fragment local for the
  authored range variable, so `value.` completion in `where` and `select`
  resolves the element type without exposing the generated lambdas.
